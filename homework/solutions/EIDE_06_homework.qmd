...
#Plotting distributions for both Sem categories
 ggplot (data= sim, 
        aes (x= Distance, fill= Sem) ) + 
        geom_density (alpha= 0.7) +  
        facet_wrap (~Sem) +  
        scale_fill_manual(values = 
                            c("Different" = "#998ec3", "Similar" = "#f1a340")) +  
  labs( x = "Distance (in milimeters)") +
  theme_minimal()


# Description of distributions: Both distributions are right-skewed with a peak around 75 ms. The semantic different distribution has a higher density of the peak than the semantic similar distribution. 
...
my_priors <-  c(prior(normal(75, 40), class = 'Intercept'),
...
#ASSESSMENT: the data does not match the Gaussian distribution of the posterior samples in terms of symmetry and shape. The data is slightly right-skewed and does not fit a bell-shaped curve with the mean centered in the middle of the distibution. 
...
sim_log<- mutate(sim, log_Distance= log(Distance))

# New model 
xmdl_log <- brm(log_Distance ~ Sem,
            family = gaussian(),
            prior = my_priors,
            data = sim_log,
            seed = 42)
...
# Still bad after the transformation, suggesting that the chosen response distribution (Gaussian) is not the most appropriate for the model.
...
# 
my_priors_strict <-  c(prior(normal(75, 1), class = 'Intercept'),
                 prior(normal(0, 1), class = 'b', coef='SemSimilar'))


#New model
xmdl_strict <- brm(Distance ~ Sem,
            family = gaussian(),
            prior = my_priors_strict,
            data = sim,
            seed = 42)
