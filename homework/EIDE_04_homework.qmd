...
#Generalized linear effects model
  model1<- glmer(chosen_voicing~correct_voicing +
...
#Checking model summary and levels of response variable
summary(model1)
levels(incomplete$chosen_voicing)

#Interpretation- FIXED EFFECTS: 
#(Intercept): The estimate (-0.023) represents the log odds of observing a "voiceless" response for the condition with underlying voiced stimuli.
# correct_voicingvoiceless: The estimate predicts an increase (0.52 log odds) in "voiceless" responses for the condition with underlying voiceless stimuli.



# Calculating p-values for the predictor(correct_voicing):

# Null-model without the relevant predictor
model1.null<- glmer(chosen_voicing~1 +
                (1+correct_voicing|listener) +
                (1+correct_voicing|voice) +
                (1+correct_voicing|item_pair), 
                data= incomplete,
...
# Comparing with likelihood ratio test
anova(model1.null, model1)

#The likelihood ratio test is revealing a significant difference for the models with and without the ciritcal predictor(χ2 (1) = 8.18, p< 0.0042). For the hypothesis, these results suggest that the predictor correct_voicing shows an effect in the prediciton of selected voicing category(voiced/voiceless) by the listener. The effect is significant, thus unikely to have occured by chance.
```
...
preds <- predict_response(model1,  terms = c( "correct_voicing"))

#Plotting predictions 
plot(preds)  +
  labs(title = "Model predictions for chosen voicing",
      subtitle = "with measure of uncertainty",
       y = " Probability of selecting the voiceless category",
       x = "Underlyingly voiced or voiceless stimuli") 
